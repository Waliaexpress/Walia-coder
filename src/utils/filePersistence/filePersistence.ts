 ...
 * - Checks environment kind (WALIA_CODER_ENVIRONMENT_KIND)
 * - Retrieves session access token
 * - Requires WALIA_CODER_REMOTE_SESSION_ID for session ID
 *
...
export async function runFilePersistence(
  ...
  const sessionId = process.env.WALIA_CODER_REMOTE_SESSION_ID
  if (!sessionId) {
    logError(
      new Error(
        'File persistence enabled but WALIA_CODER_REMOTE_SESSION_ID is not set',
      ),
    )
    return null
  }
 ...
 * and WALIA_CODER_REMOTE_SESSION_ID.
