...
async function installFromGitHub(
  ...
  const gitUrl = isEnvTruthy(process.env.WALIA_CODER_REMOTE)
 ...
 * WALIA_CODER_REMOTE) or any URL that passes validateGitUrl (https, http,
 * file, git@ ssh).
 */
function resolveGitSubdirUrl(url: string): string {
  if (/^[a-zA-Z0-9-_.]+\/[a-zA-Z0-9-_.]+$/.test(url)) {
    return isEnvTruthy(process.env.WALIA_CODER_REMOTE)
 ...
 *
 * WALIA_CODER_SYNC_PLUGIN_INSTALL=1 delegates to the full loader — that
