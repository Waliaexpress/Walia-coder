...
export function getUserAgent(): string {
  ...
  return `claude-cli/${MACRO.VERSION} (${process.env.USER_TYPE}, ${process.env.WALIA_CODER_ENTRYPOINT ?? 'cli'}${agentSdkVersion}${clientApp}${workloadSuffix})`
}

export function getMCPUserAgent(): string {
  const parts: string[] = []
  if (process.env.WALIA_CODER_ENTRYPOINT) {
    parts.push(process.env.WALIA_CODER_ENTRYPOINT)
  }
