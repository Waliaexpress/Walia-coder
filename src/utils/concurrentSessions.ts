...
function envSessionKind(): SessionKind | undefined {
  if (feature('BG_SESSIONS')) {
    const k = process.env.WALIA_CODER_SESSION_KIND
  ...
  try {
    ...
    await writeFile(
      ...
      jsonStringify({
        ...
        ...(feature('UDS_INBOX')
          ? { messagingSocketPath: process.env.WALIA_CODER_MESSAGING_SOCKET }
          : {}),
        ...(feature('BG_SESSIONS')
          ? {
              name: process.env.WALIA_CODER_SESSION_NAME,
              logPath: process.env.WALIA_CODER_SESSION_LOG,
              agent: process.env.WALIA_CODER_AGENT,
            }
          : {}),
      }),
    )
...
export async function countConcurrentSessions(): Promise<number> {
  ...
  for (const file of files) {
    ...
    if (isProcessRunning(pid)) {
      ...
      // or WALIA_CONFIG_DIR), a Windows PID won't be probeable from WSL
