 ...
 * WALIA_CODER_PROVIDER_MANAGED_BY_HOST in spawn env), strip
...
function withoutHostManagedProviderVars(
  ...
  if (!env) return {}
  if (!isEnvTruthy(process.env.WALIA_CODER_PROVIDER_MANAGED_BY_HOST)) {
    return env
  }
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // flagSettings env applied. Eligibility reads WALIA_CODER_USE_BEDROCK,
  ...
  // when WALIA_CODER_PROVIDER_MANAGED_BY_HOST is set.
 ...
 * WALIA_CODER_PROVIDER_MANAGED_BY_HOST is set — see filterSettingsEnv) and
