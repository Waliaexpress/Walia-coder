...
export const WALIA_CONFIG_DIRECTORIES = [
  'commands',
  'agents',
  'output-styles',
  'skills',
...
export type ClaudeConfigDirectory = (typeof WALIA_CONFIG_DIRECTORIES)[number]
 ...
 * 3. Can be explicitly enabled via WALIA_CODER_USE_NATIVE_FILE_SEARCH env var
 *
 * Symlink handling:
...
> {
  ...
  // - Fallback: native Node.js (when WALIA_CODER_USE_NATIVE_FILE_SEARCH is set)
  //
  // Why both? Ripgrep has poor startup performance in native builds.
  const useNative = isEnvTruthy(process.env.WALIA_CODER_USE_NATIVE_FILE_SEARCH)
