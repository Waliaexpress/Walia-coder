 ...
 * pipeline from user input to first token arrival. Enable by setting WALIA_CODER_PROFILE_QUERY=1
 *
...
const ENABLED = isEnvTruthy(process.env.WALIA_CODER_PROFILE_QUERY)
...
function getQueryProfileReport(): string {
  if (!ENABLED) {
    return 'Query profiling not enabled (set WALIA_CODER_PROFILE_QUERY=1)'
  }
