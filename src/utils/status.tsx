...
export function buildAPIProviderProperties(): Property[] {
  ...
  if (apiProvider === 'firstParty') {
    ...
    if (isEnvTruthy(process.env.WALIA_CODER_SKIP_BEDROCK_AUTH)) {
      properties.push({
  ...
  if (mtlsConfig) {
    if (mtlsConfig.cert && process.env.WALIA_CODER_CLIENT_CERT) {
      properties.push({
        label: 'mTLS client cert',
        value: process.env.WALIA_CODER_CLIENT_CERT
      });
    }
    if (mtlsConfig.key && process.env.WALIA_CODER_CLIENT_KEY) {
      properties.push({
        label: 'mTLS client key',
        value: process.env.WALIA_CODER_CLIENT_KEY
      });
    }
  }
  return properties;
