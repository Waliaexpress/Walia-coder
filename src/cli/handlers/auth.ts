...
export async function authLogin({
  ...
  const envRefreshToken = process.env.WALIA_CODER_OAUTH_REFRESH_TOKEN
  if (envRefreshToken) {
    const envScopes = process.env.WALIA_CODER_OAUTH_SCOPES
    if (!envScopes) {
      process.stderr.write(
        'WALIA_CODER_OAUTH_SCOPES is required when using WALIA_CODER_OAUTH_REFRESH_TOKEN.\n' +
...
export async function authStatus(opts: {
  ...
  const hasApiKeyEnvVar =
    (!!process.env.WALIA_API_KEY || !!process.env.ANTHROPIC_API_KEY) && !isRunningOnHomespace()
  ...
  } else if (apiKeySource === 'ANTHROPIC_API_KEY' || apiKeySource === 'WALIA_API_KEY' || hasApiKeyEnvVar) {
    authMethod = 'api_key'
  ...
  if (opts.text) {
    ...
    if (!hasAuthProperty && hasApiKeyEnvVar) {
      process.stdout.write('API key: WALIA_API_KEY\n')
    }
    if (!loggedIn) {
