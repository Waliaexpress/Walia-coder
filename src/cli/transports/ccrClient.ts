 ...
 * - Epoch management: reads worker_epoch from WALIA_CODER_WORKER_EPOCH env var
...
export class CCRClient {
   ...
   * (WALIA_CODER_SESSION_ACCESS_TOKEN env var). Callers managing multiple
  ...
  constructor(
    ...
    opts?: {
       ...
       * WALIA_CODER_SESSION_ACCESS_TOKEN (single-session callers — REPL,
   ...
   *    WALIA_CODER_WORKER_EPOCH (set by env-manager / bridge spawner)
  ...
  async initialize(epoch?: number): Promise<Record<string, unknown> | null> {
    ...
    if (epoch === undefined) {
      const rawEpoch = process.env.WALIA_CODER_WORKER_EPOCH
