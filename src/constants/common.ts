...
export function getLocalISODate(): string {
  ...
  if (process.env.WALIA_CODER_OVERRIDE_DATE) {
    return process.env.WALIA_CODER_OVERRIDE_DATE
  }
...
export function getLocalMonthYear(): string {
  const date = process.env.WALIA_CODER_OVERRIDE_DATE
    ? new Date(process.env.WALIA_CODER_OVERRIDE_DATE)
    : new Date()
