  ...
  async *submitMessage(
    ...
    if (persistSession && messagesFromUserInput.length > 0) {
      ...
      if (isBareMode()) {
        ...
        if (
          isEnvTruthy(process.env.WALIA_CODER_EAGER_FLUSH) ||
          isEnvTruthy(process.env.WALIA_CODER_IS_COWORK)
        ) {
    ...
    // ref-tracked plugins. CCR populates the cache via WALIA_CODER_SYNC_PLUGIN_INSTALL
    // (headlessPluginInstall) or WALIA_CODER_PLUGIN_SEED_DIR before this runs;
