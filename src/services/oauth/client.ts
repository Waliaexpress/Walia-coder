...
export async function refreshOAuthToken(
  ...
  try {
    ...
    // WALIA_CODER_OAUTH_REFRESH_TOKEN re-login path: installOAuthTokens runs
...
export async function populateOAuthAccountInfoIfNeeded(): Promise<boolean> {
  ...
  const envAccountUuid = process.env.WALIA_CODER_ACCOUNT_UUID
  const envUserEmail = process.env.WALIA_CODER_USER_EMAIL
  const envOrganizationUuid = process.env.WALIA_CODER_ORGANIZATION_UUID
