...
// Walia Nexus API key bridge: map WALIA_API_KEY → ANTHROPIC_API_KEY so the
// upstream LLM client receives the correct credential without any code-path change.
// This runs before all other module evaluation, which is required.
// eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
if (process.env.WALIA_API_KEY && !process.env.ANTHROPIC_API_KEY) {
  // eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
  process.env.ANTHROPIC_API_KEY = process.env.WALIA_API_KEY;
}
...
if (process.env.WALIA_CODER_REMOTE === 'true') {
  // eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
...
// eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
if (feature('ABLATION_BASELINE') && process.env.WALIA_CODER_ABLATION_BASELINE) {
  for (const k of ['WALIA_CODER_SIMPLE', 'WALIA_CODER_DISABLE_THINKING', 'DISABLE_INTERLEAVED_THINKING', 'DISABLE_COMPACT', 'DISABLE_AUTO_COMPACT', 'WALIA_CODER_DISABLE_AUTO_MEMORY', 'WALIA_CODER_DISABLE_BACKGROUND_TASKS']) {
    // eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
    process.env[k] ??= '1';
  }
}
...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.VERSION} (Walia Coder)`);
    return;
  }
