 ...
 *   1. WALIA_CODER_DISABLE_AUTO_MEMORY env var (1/true → OFF, 0/false → ON)
 *   2. WALIA_CODER_SIMPLE (--bare) → OFF
 *   3. CCR without persistent storage → OFF (no WALIA_CODER_REMOTE_MEMORY_DIR)
 *   4. autoMemoryEnabled in settings.json (supports project-level opt-out)
 *   5. Default: enabled
 */
export function isAutoMemoryEnabled(): boolean {
  const envVal = process.env.WALIA_CODER_DISABLE_AUTO_MEMORY
  ...
  if (
    ...
    !process.env.WALIA_CODER_REMOTE_MEMORY_DIR
  ) {
    return false
  }
 ...
 * Resolution order:
 *   1. WALIA_CODER_REMOTE_MEMORY_DIR env var (explicit override, set in CCR)
 *   2. ~/.claude (default config home)
 */
export function getMemoryBaseDir(): string {
  if (process.env.WALIA_CODER_REMOTE_MEMORY_DIR) {
    return process.env.WALIA_CODER_REMOTE_MEMORY_DIR
  }
 ...
 * env vars / settings.json / WALIA_CONFIG_DIR are session-stable in
