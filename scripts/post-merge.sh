...
GITHUB_SYNC_HISTORY_FILE=".github-sync-history.jsonl"
...
if [ -z "$EFFECTIVE_TOKEN" ]; then
  ...
  ENTRY=$(jq -n --arg ts "$TIMESTAMP" \
    '{"status":"skipped","reason":"GITHUB_TOKEN not set","timestamp":$ts}')
  echo "$ENTRY" > "$GITHUB_SYNC_STATUS_FILE"
  echo "$ENTRY" >> "$GITHUB_SYNC_HISTORY_FILE"
  exit 0
fi
...
if [ $PUSH_EXIT -ne 0 ]; then
  ...
  ENTRY=$(jq -n --arg ts "$TIMESTAMP" --argjson code "$PUSH_EXIT" --arg reason "$PUSH_OUTPUT" \
...
ENTRY=$(jq -n --arg ts "$TIMESTAMP" '{"status":"success","timestamp":$ts}')
echo "$ENTRY" > "$GITHUB_SYNC_STATUS_FILE"
echo "$ENTRY" >> "$GITHUB_SYNC_HISTORY_FILE"
